...
const Navbar = () => {
    ...
    return (
        <header className='fixed flex items-center md:justify-center w-full h-[6rem] md:h-[7rem] p-[1rem] bg-[var(--bgColor3)] backdrop-blur-[4px] z-[2]'>
            ...
            <Link to="/" className='logoDiv cursor-pointer flex-center ml-[5%]' onClick={handleScrollToTop}>
            ...
            <navbar className={`${menuDisplay} absolute h-[60vh] w-[80vw] rounded-[1rem] bg-[rgba(0,0,0,0.7)] top-[6.25rem] left-[50%] p-[1rem] transform-[translate(-50%)] shadow-[0_2px_4px_rgba(2,15,29,0.9)] 
                ...
                <ul className="relative w-full h-full flex flex-col items-center justify-around md:justify-end py-[2rem] md:flex-row">
                    <Link to="/"><li className='navLink' onClick={handleScrollToTop}>Home</li></Link>
                    <Link to="/tour"><li className='navLink' onClick={handleScrollToTop}>Tour</li></Link>
                    <Link to="/about"><li className='navLink' onClick={handleScrollToTop}>About</li></Link>
                    <Link to="/media"><li className='navLink' onClick={handleScrollToTop}>Media</li></Link>
                </ul>
                <div onClick={handlePopUpMenu} className="md:hidden absolute top-[1rem] right-[1.25rem] text-[2.5rem] p-[0.5rem] text-[var(--sColor)] hover:text-[var(--pColor)] transition-all duration-300">
                    <IoCloseCircle />
                </div>
            </navbar>

            <div  onClick={handlePopUpMenu} className="absolute top-[1.75rem] right-[2rem] text-[var(--pColor)] hover:text-[var(--sColor)] text-[2.5rem] md:hidden transition-all duration-300">
                <TbGridDots />
            </div>
        </header>
    )
